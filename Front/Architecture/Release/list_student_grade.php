...
while($result = mysqli_fetch_row($query_1))
{
    ...
    if(isset($result[21]) && $result[21] != 0)
        $edit_points_for_constraint_per_question[] = (float)$result[21]; // convert
    else if(isset($result[21]) && $result[21] == 0)
        $edit_points_for_constraint_per_question[] = (float)$result[21]; // convert
    else
        $edit_points_for_constraint_per_question[] = NULL; // keep it the same

    $edit_points_for_case_1_per_question[] = (float)$result[22];
    $edit_points_for_case_2_per_question[] = (float)$result[23];

    if(isset($result[24]) && $result[24] != 0)
        $edit_points_for_case_3_per_question[] = (float)$result[24];
    else if(isset($result[24]) && $result[24] == 0)
        $edit_points_for_case_3_per_question[] = (float)$result[24];
    else
        $edit_points_for_case_3_per_question[] = NULL;
    
    if(isset($result[25]) && $result[25] != 0)
        $edit_points_for_case_4_per_question[] = (float)$result[25];
    else if(isset($result[25]) && $result[25] == 0)
        $edit_points_for_case_4_per_question[] = (float)$result[25];
    else
        $edit_points_for_case_4_per_question[] = NULL;

    if(isset($result[26]) && $result[26] != 0)
        $edit_points_for_case_5_per_question[] = (float)$result[26];
    else if(isset($result[26]) && $result[26] == 0)
        $edit_points_for_case_5_per_question[] = (float)$result[26];
    else
        $edit_points_for_case_5_per_question[] = NULL;
};
...
for ($i = 0; $i < count($q_ids); $i++) // just using q_ids because everything else uses the same count...
{
    ...
    $test_case_r[] = array(); // make a new array for each question
...
$counter = 0;
while ($counter < count($q_ids))
{
    $test_case_r[$counter][0] = array(
        'case_1' => $auto_points_for_case_1_per_question[$counter],
        'comments' => $auto_comment_for_case_1_per_question[$counter]
    );

    $test_case_r[$counter][1] = array(
        'case_2' => $auto_points_for_case_2_per_question[$counter],
        'comments' => $auto_comment_for_case_2_per_question[$counter]
    );
    
    if (!empty($auto_comment_for_case_3_per_question[$counter]))
    {
        $test_case_r[$counter][2] = array(
            'case_3' => $auto_points_for_case_3_per_question[$counter],
            'comments' => $auto_comment_for_case_3_per_question[$counter]
        );
    }

    if (!empty($auto_comment_for_case_4_per_question[$counter]))
    {
        $test_case_r[$counter][3] = array(
            'case_4' => $auto_points_for_case_4_per_question[$counter],
            'comments' => $auto_comment_for_case_4_per_question[$counter]
        );
    }

    if (!empty($auto_comment_for_case_5_per_question[$counter]))
    {
        $test_case_r[$counter][4] = array(
            'case_5' => $auto_points_for_case_5_per_question[$counter],
            'comments' => $auto_comment_for_case_5_per_question[$counter]
        );
    }
    $counter += 1;
}
